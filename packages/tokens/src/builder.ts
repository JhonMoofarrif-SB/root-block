...
/**
 * Formato personalizado para CSS Variables con prefijo --b-ui-
 * Genera variables CON prefijo --b-ui- para evitar colisiones
 */
  ...
  format: ({ dictionary, options }) => {
    ...
    const tokens = dictionary.allTokens
      .map((token) => {
        ...
        // Agregar prefijo --b-ui- a todas las variables
        return `  --b-ui-${name}: ${value};`;
      })
      .join('\n');
 ...
 *
 * Variables con prefijo --b-ui- (Bolivar UI)
 * Todas las marcas tienen las mismas variables con diferentes valores
 * Ejemplo: --b-ui-color-primary-base, --b-ui-typography-fontFamily
 */
...
export function getTokensPath(config: TokenConfig): string {
  return \`@bolivar-ui/tokens/\${config.brand}-\${config.theme}.css\`;
}
...
export function getTokensPath(brand, theme) {
  return \`@bolivar-ui/tokens/\${brand}-\${theme}.css\`;
}
...
async function build() {
  console.log('🚀 Building Bolivar UI Design Tokens...\n');
